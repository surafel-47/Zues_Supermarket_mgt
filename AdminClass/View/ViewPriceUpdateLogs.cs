...
{
    public partial class ViewPriceUpdateLogs : Form
    {
        SqlConnection sqlCon;
        DataTable dt;
        public ViewPriceUpdateLogs()
        {
            InitializeComponent();
        ...
        private void LoadPriceLogsTable()
        {
            makeConnection();// Making Connection

            try
            {
                SqlCommand sqlCmd = new SqlCommand("Exec USP_ViewPriceUpdateLogs", sqlCon);
                SqlDataReader reader = sqlCmd.ExecuteReader();
                dt = new DataTable();
                dt.Load(reader);
                priceUpdateLogTbl.DataSource = dt;
            }
            catch (Exception e)
            {
